define('profile',['reg','users'],function(reg,users){
	...
	let newAvatarLink = document.querySelector('.left__profile-link');

	// Форма изменения аватара
	let newAvatar = document.querySelector('.new-avatar');

	// Кнопка закрытия форма изменения аватара
	let newAvatarClose = document.querySelector('.new-avatar__form-close');

	// Зона для загрузки файла
	let newAvatarUpload = document.querySelector('.new-avatar__upload');

	// Область вывода ошибки в форме загрузки аватара
	let newAvatarError = document.querySelector('.new-avatar__error');

	// Файл с изображением нового аватара
	let newAvatarFile = "";

	// Кнопка загрузка изображения
	let newAvatarSubmit = document.querySelector('.new-avatar__form-submit');

	// Аватар пользователя в левой части
	let profileAvatar = document.querySelector('.left__profile-avatar');
